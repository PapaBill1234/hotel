...
@foreach($stickers as $i => $row)
				<li id="subcategory-1-{{ (int) $row->category_id }}-stickers" class="subcategory{{ ($selectedSub === (int) $row->category_id || $i === 0) ? '-selected' : '' }}">
...
<div id="webstore-content-container">
	<div id="webstore-items-container">
		...
		<div id="webstore-items">
			@include('hotel.habblet.store-items', ['homes' => $homes, 'items' => $items])
		</div>
	</div>
	<div id="webstore-preview-container">
		...
		<div id="webstore-preview">
			@if(!empty($preview))
				@include('hotel.habblet.store-preview', ['homes' => $homes, 'row' => $preview, 'credits' => $homes->credits(), 'inHotel' => $homes->inHotel()])
			@endif
		</div>
	</div>
</div>
